...
def _report_html(
    ...
    *,
    ...
    return render_test_report(
        "6.3.4", "Тест на динамику ключевой метрики",
        ...
        "отрицательное — рост.",
        (),
        [("green", f"Изменение метрики не более {format_report_number(green_threshold, 0, percent=True)}"),
         ("yellow", f"Изменение метрики больше {format_report_number(green_threshold, 0, percent=True)} "
          f"и меньше {format_report_number(c_min_threshold, 0, percent=True)}"),
         ("red", f"Изменение метрики {format_report_number(c_min_threshold, 0, percent=True)} и более"),
         ("gray", "Динамика метрики не вычислена; причина указана в результате")],
        algorithm=(
            "Сформировать единицы оценки и агрегировать ключевую метрику по правилам выбранного режима.",
            "Вычислить изменение метрики мониторинга относительно первичной валидации в процентах.",
        ),
